...
@Component({
  selector: 'app-monitor-ano',
  templateUrl: './monitor-ano.component.html',
  styleUrls: ['./monitor-ano.component.scss']
})
export class MonitorAnoComponent implements OnInit {
  ...
  openDialog(): void {
    ...
    let dialogRef = this.dialog.open(DialogSettingsComponentAno, {
      // height: '250px',
      width: '450px'
      ...
      .subscribe(() => {
        ...
        if (index == -1) {
          ...
          if (index >= this.lista.length - 1) {
            // console.log('index: ' + index + ' lista: ' + this.lista.length);
            index = 0;
            this._alternadorservice.capturaI(index);
          ...
          if (this._alternadorservice.retorna2() && ((index+1) % 3 == 0) && (index > 0)) {
            index++;
        // console.log('index: ' + index);
        this._alternadorservice.capturaI(index);
            this.router.navigate(['grandesnumeros']);
            this.alive = false;

            } else
            if (this._alternadorservice.retorna3() && ((index+1) % 3 == 0) && (index > 0)) {
              index++;
          // console.log('index: ' + index);
          this._alternadorservice.capturaI(index);
...
@Component({
  selector: 'dialog-settings-component-ano',
  templateUrl: 'dialog-settings-component.html',
})
export class DialogSettingsComponentAno {
  ...
  constructor(
    public dialogRef: MatDialogRef<DialogSettingsComponentAno>,
    private router: Router,
    public _alternadorservice: AlternadorService,
